...
```mermaid
flowchart LR
  Claude["Claude Code CLI<br/>(in your terminal)"]
  ...
  Claude -->|fires hook events| Hook
  Claude -->|appends records| JSONL
...
You run `claude` in a terminal. It starts a session with a unique session ID. Everything Claude does (reading files, running commands, asking permission, completing a turn) emits two kinds of data: a hook event (synchronous, structured) and a JSONL transcript line (asynchronous, append-only).

### 2. The hook script

When you enable hooks in Pixel Agents settings, the extension writes entries into `~/.claude/settings.json` that tell Claude to run our script on every event of interest. The script is one bundled JavaScript file (`~/.pixel-agents/hooks/claude-hook.js`) that reads the event payload from stdin and POSTs it to the local Pixel Agents server with a Bearer token.

### 3. The JSONL transcript

Whether or not hooks are enabled, Claude also writes session activity to a JSONL file at `~/.claude/projects/<workspace-hash>/<session-id>.jsonl`. Each line is a JSON record describing a tool call, a tool result, a turn boundary, etc.
...
The UI knows nothing about Claude or hooks or JSONL. It speaks the [AsyncAPI protocol](/learn/protocol). Any client that speaks the protocol gets the same experience.

## What happens when you click "+ Agent"

In VS Code:

1. The extension opens a new terminal named `claude`.
2. The extension runs `claude --session-id <uuid>` inside it. The `<uuid>` is generated client-side so Pixel Agents knows exactly which session ID to watch for.
3. The extension polls `~/.claude/projects/<hash>/<uuid>.jsonl` until the file appears.
...
In standalone, there's no "+ Agent" button (no IDE-managed terminal to spawn). Agents appear when you start `claude` yourself in any terminal under the watched directory.

## What happens when Claude reads a file

With hooks enabled:

1. Claude fires a `PreToolUse` hook with `tool_name: 'Read'` and the file path.
2. Our hook script POSTs it to the server.
3. The server's `claudeProvider.normalizeHookEvent` translates it to `AgentEvent { kind: 'toolStart', toolName: 'Read', input: {...} }`.
4. The runtime broadcasts `agentToolStart` over WebSocket / postMessage.
5. The UI shifts the character to the reading animation.

When Claude finishes, `PostToolUse` fires and the cycle reverses.

Without hooks, the same sequence is reconstructed from JSONL polling, with a ~500ms delay.

## What happens when Claude asks for permission
...
Without hooks: a heuristic timer fires 7 seconds after a non-exempt tool starts. If the tool hasn't completed by then, the bubble appears. This is the source of occasional false positives on slow tools like `WebFetch`. It's why hooks are recommended.
...
- `server/` - the Fastify server, the AgentRuntime, the file watcher, the bundled Claude provider.
...
- [Concepts](/learn/concepts) - vocabulary you'll see in the docs.
- [The office](/learn/the-office) - the UI in detail.
- [Agent lifecycle](/learn/agent-lifecycle) - the lifecycle of a single agent.
- [Architecture](/learn/architecture) - the four packages and how they talk.
- [Build extensions](/build) - if you want to add a new AI CLI, alt UI, or new IDE host.
